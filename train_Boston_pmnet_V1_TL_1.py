...
from config.config_Boston_pmnet_V1_TL_3 import config_Boston_pmnet_V1_TL_3 as Config
...
def helper(cfg,writer,phase='train', data_root = 'C:/Users/ABMAN23_ML/Desktop/Joohan/Boston/', model_to_eval=''):

    # data_root = 'C:/Users/ABMAN23_ML/Desktop/Joohan/Boston/'
    
    if cfg.sampling == 'exclusive':
        num_of_maps = 3143
        ddf = pd.DataFrame(np.arange(1,num_of_maps))
        ddf.to_csv(os.path.join(data_root,'Data_coarse_train.csv'),index=False)

        data_boston_train = PMnet_boston(csv_file = os.path.join(data_root,'Data_coarse_train.csv'), dir_dataset=data_root)

        dataset_size = len(data_boston_train)

        train_size = int(dataset_size * cfg.train_ratio)
        # validation_size = int(dataset_size * 0.1)
        test_size = dataset_size - train_size
        train_dataset, test_dataset = random_split(data_boston_train, [train_size, test_size], generator=torch.Generator(device='cuda'))
...
if __name__ == "__main__":
    ...
    model_to_eval = sys.argv[3] if len(sys.argv) >3 else ''
